...
def _ssim(img1, img2, img3, window, window_size, channel, size_average=True):
    ...
    # nen_2=mu2_sq
    # nen_3=mu3_sq
    # ms_ssim_module = CMS_SSIM(data_range=255, size_average=True, channel=1)
    # MSSIM12=ms_ssim_module(img1, img2)
    # MSSIM13=ms_ssim_module(img1, img3)
    if size_average:
        # ssim_map_std = torch.where(torch.abs((x2))>=torch.abs((x3)), ssim_map_12, ssim_map_13)
        # ssim_map_mu = mu2/(mu2+mu3)*MSSIM12+ mu3/(mu2+mu3)*MSSIM13
...
def ssim(img1, img2, img3, window_size=3, size_average=True):
    (_, channel, _, _) = img1.size()
    window = create_window(window_size, channel)
