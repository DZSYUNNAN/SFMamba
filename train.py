...
def train_fusion(num=0, logger=None):
    ...
    train_dataset = Fusion_dataset('train',length=11661)
    ...
    for epo in range(0, epoch):
        ...
        for it, (image_vis, image_ir, image_f1, image_f2) in enumerate(train_loader):
            try:
                fusionmodel.train()
                image_vis = Variable(image_vis).cuda()
                # image_vis_ycrcb = image_vis[:,0:1:,:,:]
                image_ir = Variable(image_ir).cuda()
                image_f1 = Variable(image_f1).cuda()
                image_f2 = Variable(image_f2).cuda()
            ...
            loss_fusion,  loss_in, ssim_loss, loss_grad= criteria_fusion(
                ...
                fusion_image, i=num, labels_1=image_f1, labels_2=image_f2
            )
            ...
            if now_it % 10 == 0:
                ...
                logger.info(msg)
                st = ed
        fusion_model_file = os.path.join(modelpth, 'fusion_model_10ssim101_2kd.pth')
